...
tab_correlation_mod <- function(id, label = "correlation") {
  ...
  tabItem(tabName = "correlation",
          ...
          fluidRow(box(title = NULL, width = 3,
                       ...
                       selectInput(ns("indicator1"), label = "Select x-axis (horizontal) variable", choices = c("Reception: Prevalence of overweight & obesity",
                                                                                               "Year 6: Prevalence of overweight & obesity",
                                                                                               "Index of Multiple Deprivation (IMD) Score",
                                                                                               "Fast food rate per 1000"), multiple = FALSE, selected = "Fast food rate per 1000"),
                       selectInput(ns("indicator2"), label = "Select y-axis (vertical) variable", choices = c("Reception: Prevalence of overweight & obesity",
                                                                                               "Year 6: Prevalence of overweight & obesity",
                                                                                               "Index of Multiple Deprivation (IMD) Score",
                                                                                               "Fast food rate per 1000"), multiple = FALSE, 
                                   selected = "Year 6: Prevalence of overweight & obesity)")),
                   box(title = "Correlation graph", width = 9,
                       plotlyOutput(ns("graph")),
                       htmlOutput(ns("text_imd")))),
                   box(title = "Guidance and caveates", width = 12, 
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      observe({
        ...
        rv_correlation$graph <- graph_data(data = correlation_data(),
      ...
      output$graph <- renderPlotly({
        ...
        ind_name1 <- get_ind_name(data = correlation_data(),
                                 indicator = input$indicator1)

        ind_name2 <- get_ind_name(data = correlation_data(),

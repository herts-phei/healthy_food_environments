...
filtered_table_data <- function(data, area, comparator) {
  ...
  df <- df %>%
    ...
    dplyr::mutate(fast_food_diff = ifelse(is.na(fast_food_diff), "NA value", fast_food_diff), # TODO diff in why this is NA no value vs supressed show differently as in should show as 0 pre 1000
      ...
      label_text_temp = case_when(Herts_quintile == 1 ~ "Area is in the most deprieved quintile. ",
        TRUE ~ ""),
      label_text = case_when(agg2 == 1 ~ "1 indicator significantly worse than comparator.",
        agg2 == 2 ~ "2 indicator significantly worse than comparator.",
        agg2 == 3 ~ "3 indicator significantly worse than comparator.",
        agg2 == 4 ~ "4 indicator significantly worse than comparator.",
        agg2 == 5 ~ "5 indicator significantly worse than comparator.",
        TRUE ~ ""),
